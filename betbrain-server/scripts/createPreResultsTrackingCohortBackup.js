/**
 * Pre Results Tracking Cohort fix backup — prod snapshot only, no mutations.
 * Usage: node betbrain-server/scripts/createPreResultsTrackingCohortBackup.js
 */
import fs from "fs";
...
const backupDir = path.join(ROOT, "safe-backups", `pre-results-tracking-cohort-${ts}`);
...
async function main() {
  ...
  const manifest = {
    ...
    source: SOURCE,
    purpose: "pre-results-tracking-cohort-v1",
    endpoints: {},
    verification: {},
  };
  ...
  const props0623 = (tracked.props || tracked).filter((p) => p.slateDate === "2026-06-23");
  ...
  manifest.verification = {
    ...
    tracked0623: {
      propCount: props0623.length,
      players: props0623.map((p) => p.player).sort(),
    },
    lab0621Safe:
  ...
  fs.writeFileSync(
    ...
    `# Pre Results Tracking Cohort backup

- **Captured:** ${manifest.capturedAt}
- **Source:** ${SOURCE}
- **Lab 06/21:** ${lab0621.propCount} props, ${lab0621.record}, ${lab?.reportStatus || lab?.status || "?"}
- **06/23 tracked:** ${props0623.length} props
- **lab0621Safe:** ${manifest.verification.lab0621Safe}
`
  );
