...
type WnbaControlledBestSixScreenProps = {
  /** Home tab: Tomorrow Best 6 only, no Today section. Explore: full date tabs. */
  variant?: "home" | "explore";
};

export default function WnbaControlledBestSixScreen({
  variant = "explore",
}: WnbaControlledBestSixScreenProps) {
  ...
  const bestSixCards = useMemo(() => {
    ...
    const scopedPool =
      effectiveDateView === "full_board"
        ? displayPool
        : filterBestSixByDateView(displayPool, effectiveDateView);
    return prepareBestSixDisplayCards(scopedPool, topPickBadgeMap);
  }, [bestSixDisplayWNBA, bestSixWNBA, topPickBadgeMap, effectiveDateView]);

  const groupedGames = useMemo(() => {
    const sections = groupByDayBucket(wnbaGames);
    if (isHome) {
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        <View style={[styles.header, isHome && styles.headerHome]}>
          <Text style={[styles.title, isHome && styles.titleHome]}>
            {isHome ? "CourtEdge" : "WNBA Props"}
          </Text>
          <Text style={styles.subtitle}>
            {isHome ? "Tomorrow — Controlled Best 6" : "Controlled Best 6"}
          </Text>
        ...
        {!isHome ? (
          ...
          <View style={styles.homeTomorrowBanner}>
            ...
            <Text style={styles.homeTomorrowSubtext}>
              Next slate props · Top 2 also appear on Top tab · Rollover → Results
            </Text>
          </View>
        )}
        ...
        <TouchableOpacity
          onPress={runRefresh}
          style={[styles.refreshButton, isHome && styles.refreshButtonHome]}
        ...
        {!loading && !loadError && bestSixCards.length === 0 && effectiveDateView !== "full_board" && (
          <View style={styles.emptyCard}>
            <Text style={styles.emptyTitle}>
              No Controlled Best 6 for {formatDateViewLabel(effectiveDateView)}.
            </Text>
