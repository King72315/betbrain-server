...
export async function selectApiBackend(options = {}) {
  ...
  if (config.mode === "local") {
    const probe = await probeLocalHealth(config.localUrl, {
      fetchFn,
      timeoutMs,
    });
    if (!probe.ok) {
  ...
  const probe = await probeLocalHealth(config.localUrl, { fetchFn, timeoutMs });
  if (probe.ok) {
    return {
      ...base,
      activeBackend: "LOCAL",
...
export function formatBackendBadgeLabel(diag) {
  ...
  if (diag.activeBackend === "LOCAL") return "Backend: LOCAL";
  if (diag.fallbackUsed) return "Backend: RENDER (Local unavailable)";
